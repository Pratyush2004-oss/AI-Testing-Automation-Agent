...
import { db } from "@/db";
import { TestCaseTable, repositories, users } from "@/db/schema";
...
export async function POST(req: NextRequest) {
    try {
        ...
        const { testCaseId, baseUrl, mode = "generate", customPrompt = "", userId } = body;
        ...
        const [testCase] = await db
            .select()
            .from(TestCaseTable)
            .where(eq(TestCaseTable.id, testCaseId));
        ...
        if (forceRegenerate) {
            ...
            const prompt = `
                    ...
                    For auth pages, NEVER use a global locator like page.getByRole('button', { name: /Login/i }) because it often matches multiple elements.
                    Always scope auth actions to the nearest form first, for example:
                    const authForm = page.locator('form').first();
                    const loginIdentifier = 'use the exact email/mobile/username provided in the instruction section';
                    const loginPassword = 'use the exact password provided in the instruction section';
                    await authForm.locator('input[type="email"], input[type="tel"], input[type="text"], input[name*="email" i], input[name*="mobile" i], input[name*="phone" i], input[name*="user" i], #email, #mobile, #phone, #username').first().fill(loginIdentifier);
                    await authForm.locator('input[type="password"], input[name="password"], #password').first().fill(loginPassword);
                    await authForm.getByRole('button', { name: /sign in|log in|login|continue|submit|next/i }).click();
                    If a button text is duplicated, use the most specific parent scope available, then use .first() or .nth(0) only as a last resort.
                    Never use getByLabel(/password/i) on login forms, because password visibility toggles or helper buttons can make the accessible name ambiguous.
                    Login may be email-based, mobile-number-based, username-based, or passwordless.
                    Prefer the visible input field that matches the provided instruction credentials, and if the page uses a mobile number field use input[type="tel"] or a phone/mobile selector before falling back to text inputs.
                    Prefer exact button names and labels over loose /Login/i regex matches.
                    ...
                    assert(bodyText.toLowerCase().includes(String(testCase?.expectedResult ?? '').toLowerCase().replace(/' / g, "\'") + ' '), 'Expected result state not matched');
            ...
            // Normalize common auth selectors so generated scripts do not fail on
            // password visibility toggles or duplicate accessible names.
            generatedCode = generatedCode.replace(
                /getByLabel\(\s*(?:['"`]?\/?password\/?i?|['"`]Password['"`])\s*\)/gi,
                "locator('input[type=\"password\"], input[name=\"password\"], #password').first()"
            );
            generatedCode = generatedCode.replace(
                /getByLabel\(\s*(?:['"`]?\/?email\/?i?|['"`]Email['"`])\s*\)/gi,
                "locator('input[type=\"email\"], input[name=\"email\"], #email').first()"
            );
            generatedCode = generatedCode.replace(
                /getByRole\(\s*['"]button['"]\s*,\s*\{\s*name:\s*\/Login\/i\s*\}\s*\)/gi,
            ...
            await db
                ...
                .where(eq(TestCaseTable.id, testCase.id));
        }
        ...
        try {
            ...
            // update the credits of the user in database
            if (updateResult) {
                await db
                    .update(users)
            ...
            // if test case is runned, update the credits of the user in database
            if (updateResult) {
                await db
                    .update(users)
                    .set({ credits: sql`${users.credits} - 100` })
