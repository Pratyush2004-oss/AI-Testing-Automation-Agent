...
type Props = {
    ...
    repository: USERREPOTYPE; // Connected repository config
...
export default function TestExecutionModal({ isOpen, onClose, testCases, repository, refresh }: Props) {
    ...
    useEffect(() => {
        if (isOpen && testCases.length > 0) {
            ...
            setBaseUrl(repository?.targetDomain || repository?.targetDomain || "http://localhost:3000");
    ...
    return (
        <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
            <DialogContent className="max-w-5xl h-[90vh] flex flex-col p-6 gap-4 bg-black rounded-2xl shadow-2xl border overflow-hidden select-none">
                <DialogHeader className="border-b pb-4 flex flex-row items-center justify-between shrink-0">
                    <div>
                        <DialogTitle className="text-2xl font-bold text-gray-200 flex items-center gap-2">
                ...
                <div className="flex flex-col bg-gray-800 p-4 rounded-2xl border border-gray-200/80 gap-3 shrink-0">
                    <div className="flex flex-col sm:flex-row gap-4 items-end">
                        <div className="flex-1 space-y-1.5">
                            ...
                            <Input
                                ...
                                className="border-gray-300 font-mono text-sm shadow-xs h-10"
                            />
                        </div>
                        <div className="flex gap-2.5">
                            ...
                            {!isExecuting ? (
                                <Button
                                    ...
                                    className="h-10 rounded-md bg-black hover:bg-primary/95 text-white hover:text-black shadow-md font-medium px-6 gap-2"
                                >
                    ...
                    {showOptions && (
                        <div className="pt-3 border-t border-gray-200/60 grid grid-cols-1 md:grid-cols-3 gap-5 animate-in fade-in slide-in-from-top-2 duration-200 items-center">
                            {/* Execution Mode Segment */}
                            <div className="md:col-span-1 space-y-1.5">
                                <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider flex items-center gap-1.5">Run Mode</span>
                                <div className="grid grid-cols-2 bg-gray-800/60 p-1 rounded-lg border border-gray-200">
                                    <button
                                        type="button"
                                        disabled={isExecuting}
                ...
                <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-5 overflow-hidden">
                            ...
                            return (
                                <div
                                    ...
                                    className={`p-3 rounded-lg border cursor-pointer transition-all ${isActive
                                        ? "bg-gray-900 border-primary shadow-sm ring-1 ring-primary/20"
                                        : "bg-gray-800/90 border-gray-200 hover:border-gray-300 shadow-xs"
                                        }`}
                                >
                                    ...
                                    <div className="flex justify-between items-center">
                                        <Badge variant="outline" className="text-[10px] font-mono capitalize bg-gray-800">
                                            {tc.type}
                                        </Badge>
                    ...
                    <div className="md:col-span-2 border rounded-xl flex flex-col  overflow-hidden shadow-sm">
                ...
                <div className="border-t pt-4 flex justify-end gap-3 shrink-0">
                    ...
                    disabled={isExecuting} className="h-10 font-medium px-5">
